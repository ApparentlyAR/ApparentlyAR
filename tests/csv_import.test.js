...
describe('CSV Import Block', () => {
  beforeEach(() => {
    jest.clearAllMocks();
    try { delete require.cache[require.resolve('../src/blocks/csv_import.js')]; } catch (_) {}
  ...
  describe('Block Definition', () => {
    ...
    test('should register csv_import_extension (no-op)', () => {
      require('../src/blocks/csv_import.js');
      // Soft assertion: extension registration should be attempted
      expect(typeof Blockly.Extensions.register).toBe('function');
    });
  });
  ...
  describe('Integration Tests', () => {
    test('should work end-to-end with file upload and code generation', (done) => {
      ...
      // Provide a proper FileList to input.files in jsdom
try {
  const dt = new dom.window.DataTransfer();
  dt.items.add(mockFile);
  Object.defineProperty(fileInput, 'files', { value: dt.files, configurable: true });
} catch (e) {
  Object.defineProperty(fileInput, 'files', {
    configurable: true,
    get: () => ({ 0: mockFile, length: 1, item: (i) => (i === 0 ? mockFile : null) })
  });
}
